...
export async function start({ mongo }) {
  const db = mongo.db();
  ...
  const { insertedId } = await JournalCollection.insertOne({
    ...
    since: mostRecentJournalEntry?.started || new Date(0),
  });
...
export async function reportFinalStatus({ status }, { mongo, journalEntry }) {
  const db = mongo.db();
  const JournalCollection = db.collection("journal");
  await JournalCollection.updateOne(
    { _id: journalEntry._id },
    {
      $set: { status, finished: new Date() },
    }
  );
  console.log(`Updated Journal entry with status ${status}`, journalEntry);
}
